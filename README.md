...
``` 
python3 ./routines/verifier/train.py  --model "ms-gan" -dataset "male"
```

The training script will save in ```./data/vs_mv_models/{net}/v{xxx}/{dataset}```: 
...
- the statistics on the training ```stats.json``` (i.e., performance (losses, accuracies), history (fake, real), model type, args). 
...
--dataset 'female'                 (gender dataset in ['female', 'male', 'neutral'])
...
``` 
python3 -u ./routines/gan/preview.py --net "ms-gan" --dataset "male" --version 0
```
...
``` 
python3 -u ./routines/gan/griffin_lim_preview.py --net "ms-gan" --dataset "male" --version 0
```

This script will create randomly-generated audios by inverting the fake spectrograms through the Griffin-Lim algorithm. 
The audio files will be saved in ```./data/vs_mv_models/{net}/v{xxx}/{dataset}/gla_samples```. 
