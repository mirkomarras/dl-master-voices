    ...
    def build(self, input_x, input_y, n_classes=0, n_filters=24, noises=None, cache=None, augment=0, n_seconds=3, sample_rate=16000):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('conv1'):
                conv1_1 = tf.layers.conv2d(filters=96, kernel_size=[7, 7], strides=[2, 2], padding='SAME', reuse=self.reuse, name='cc1')(self.input_s)
                conv1_1 = tf.layers.batch_normalization(training=self.phase, name='bbn1', reuse=self.reuse)(conv1_1)
