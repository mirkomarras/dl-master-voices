...
def eval_any(args, sess, model, noises, thresholds, enrol_size, trials, target_paths, target_ordered_labels, target_embs, target_males, target_females, utterance_per_person, file):
    results = []
...
def eval_avg(args, sess, model, noises, thresholds, enrol_size, trials, target_paths, target_ordered_labels, target_embs, target_males,  target_females, utterance_per_person, file):
    results = []
    for n_thr, thr in enumerate(thresholds):
    ...
    with tf.Session(config=config) as sess:
        ...
        for index, file in enumerate(f_voices):
            ...
            r_any = eval_any(model=model, thresholds=[args.thr_eer, args.thr_far1], enrol_size=10, trials=1, target_paths=np.copy(test_paths), target_ordered_labels=np.copy(test_ordered_labels),
                     ...
                     file=file, args=args, noises=noises, sess=sess)
