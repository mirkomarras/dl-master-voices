...
class ThinResNet34(Model):
    ...
    def __init__(self, name='thin_resnet', id='', target_length=DEFAULT_LENGTH):
    ...
    def build(self, classes=0, embs_name='embs', embs_size=512, loss='softmax', aggregation='gvlad', vlad_clusters=10, ghost_clusters=2, weight_decay=1e-3, mode='train'):
        ...
        # Conv 2
        x2 = self.conv_block_2d(x1, 3, [48, 48, 96], stage=2, block='a', strides=(1, 1), weight_decay=weight_decay, trainable=True)
        x2 = self.identity_block_2d(x2, 3, [48, 48, 96], stage=2, block='b', weight_decay=weight_decay, trainable=True)

        # Conv 3
        x3 = self.conv_block_2d(x2, 3, [96, 96, 128], stage=3, block='a', strides=(2, 2), weight_decay=weight_decay, trainable=True)
        x3 = self.identity_block_2d(x3, 3, [96, 96, 128], stage=3, block='b', weight_decay=weight_decay, trainable=True)
        x3 = self.identity_block_2d(x3, 3, [96, 96, 128], stage=3, block='c', weight_decay=weight_decay, trainable=True)

        # Conv 4
        x4 = self.conv_block_2d(x3, 3, [128, 128, 256], stage=4, block='a', strides=(2, 2), weight_decay=weight_decay, trainable=True)
        x4 = self.identity_block_2d(x4, 3, [128, 128, 256], stage=4, block='b', weight_decay=weight_decay, trainable=True)
        x4 = self.identity_block_2d(x4, 3, [128, 128, 256], stage=4, block='c', weight_decay=weight_decay, trainable=True)

        # Conv 5
        x5 = self.conv_block_2d(x4, 3, [256, 256, 512], stage=5, block='a', strides=(2, 2), weight_decay=weight_decay, trainable=True)
        x5 = self.identity_block_2d(x5, 3, [256, 256, 512], stage=5, block='b', weight_decay=weight_decay, trainable=True)
        x5 = self.identity_block_2d(x5, 3, [256, 256, 512], stage=5, block='c', weight_decay=weight_decay, trainable=True)
        ...
        self.model = tf.keras.Model(inputs=input_layer, outputs=output_layer, name='thinresnet34_{}_{}'.format(loss, aggregation))
