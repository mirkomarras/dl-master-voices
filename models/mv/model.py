...
class SiameseModel(object):
    ...
    def build(self, fft_size=256):
        ...
        if self.gan is not None:
            logger.info('Optimization through gan')
            ...
            logger.info('Optimization through spectrum')
    ...
    def batch_optimize_by_path(self, seed_voice, train_data, test_gallery, settings=None):
        ...
        for iter in range(n_seed_voices): # For each audio sample to batch_optimize_by_path
            ...
            logger.debug(f'Created master voice seed {input_sv.shape}')
    ...
    def optimize(self, input_sv, train_data, test_gallery=None, settings=None):
        ...
        for epoch in range(settings.n_epochs):  # For each optimization epoch
            ...
            logger.debug('Imp@EER m={:.3f} f={:.3f} | Imp@FAR1 m={:.3f} f={:.3f} | opt time {:.1f} + val time {:.1f}'.format(
                results[0]["m"], results[0]["f"], results[1]["m"], results[1]["f"], opt_time, val_time))
    ...
    def save(self, iter, input_sv, input_mv, input_avg, input_std, performance_stats, test_gallery, filename=''):
        ...
        # filename_format = net + '_sims_' + test_gallery.pop_file + '_' + mv_set + '_' + 'eer' + '_' + 'any'
        net = self.params['netv'].replace('/', '_')
        # mv_set = self.params
        filename_format = f'{net}_{test_gallery.pop_file}_{{}}_any.npz'

        for thr in ('eer', 'far1'):
            results = {'sims': self.sims[thr], 'imps': self.imps[thr], 'gnds': self.gnds[thr]}
            np.savez(os.path.join(self.dir_full, filename_format.format(thr)), results)
            # self.sims[thr].to_csv(os.path.join(self.dir_full, filename_format.format('sims', thr)))
            # self.imps[thr].to_csv(os.path.join(self.dir_full, filename_format.format('imps', thr)))
            # self.gnds[thr].to_csv(os.path.join(self.dir_full, filename_format.format('gnds', thr)))
