...
class Model(object):
    ...
    def build_model(self, input_x, input_y, num_classes, output_dir):
        ...
        with self.graph.as_default():
            ...
            self.input_x = input_x if input_x is not None else tf.placeholder(tf.float32, [None, 512, 300, 1], name="input_x")
            ...
            with tf.variable_scope('conv4'):
                ...
                # conv4_3 = tf.layers.average_pooling2d(conv4_3, pool_size=[1, tf.shape(conv4_3)[2]], strides=[1,1], name='apool4')
            ...
            for epoch in range(n_epochs):
                ...
                for minibatch_idx in range(minibatch_count):
                    ...
                    if minibatch_idx % print_interval == 0:
                        print("\rStep %4.0f / %4.0f | AvgLoss: %3.4f | AvgAcc: %3.7f | AvgDiskTime: %3.1f | AvgGPUTime: %3.1f | ElapsedTime: %3.1f" % (minibatch_idx+1, minibatch_count, total_loss / (minibatch_idx+1), total_accuracy / (minibatch_idx+1), 0, 0, time.time() - start_time), end='')
