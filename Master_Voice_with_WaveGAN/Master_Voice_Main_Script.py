...
def run_model(ir_speaker_dir, ir_room_dir, ir_mic_dir,
              ...
              post_processing, save_dir, total_runs):
    ...
    print('Max similarity factor:', max_similarity)
    ...
    tf.reset_default_graph() # Reset current tf.Graph()
    ...
    # Defining False Acceptance Count (FAC)
    ...
    # Given the input parameters, the model is run for a total of n times (with m iterations each)
    counter = 0
    ...
    for n in range(total_runs):
        ...
        # File path for saving the data (Cost, FAC, Audio, Spectrogram)
        if utterance_type == 'male':
            first_path_value = 'Male'
        elif utterance_type == 'female':
            first_path_value = 'Female'

        if post_processing:
            second_path_value = 'Yes-PP'
        else:
            second_path_value = 'No-PP'

        net_path_value = first_path_value + '_' + second_path_value + '_' + str(counter)
        save_dir_path = os.path.join(save_dir, net_path_value)
        ...
        plt.savefig(os.path.join(save_dir_path, 'Data_plot.png'))

        '''
        Audio and Spectrogram (Starting and Master latent vector)
        '''

        # Generate audio and spectrogram for Starting and Master (z) value
        if post_processing:
        ...
        plt.savefig(os.path.join(save_dir_path, 'Spec_Starting.png'))

        plt.figure()
        plt.imshow(master_spectrogram[0,:,:,0])
        plt.title('Spectrogram (Master _z)')
        plt.savefig(os.path.join(save_dir_path, 'Spec_Master.png'))

        # Save audio
        write(os.path.join(save_dir_path, 'Audio_Starting.wav'), 16000, starting_audio[0,:,0])
        write(os.path.join(save_dir_path, 'Audio_Master.wav'), 16000, master_audio[0,:,0])
        print('Training: End')
...
def main():
    ...
    parser.add_argument('--iterations', dest='n_iterations', default=1000, type=int, action='store',
    ...
    run_model(args.ir_speaker_dir, args.ir_room_dir, args.ir_mic_dir,
                  ...
                  args.post_processing, args.save_dir, args.total_runs)
